...
- ResultsScreen accuracy-by-type shows only percentage + colored bar (no hit/miss/false-alarm counts). Max combo widget removed. Save status only shown for `queued` and `error` states (no "Saving..."/"Session saved")
