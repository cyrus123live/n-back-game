      ...
      {loading ? (
        <div className="space-y-2">
          {[1, 2, 3].map((i) => (
            <div key={i} className="card animate-pulse">
              <div className="h-16 bg-secondary-surface rounded" />
            </div>
          ))}
        </div>
      ) : sessions.length === 0 ? (
        ...
        <>
                ...
                <LineChart data={chartData.data}>
                  ...
                  {chartData.levels.map((level) => (
                    <Line
                      ...
                      type="monotone"
                      dataKey={`${level}-back`}
                      stroke={LEVEL_COLORS[level] || '#538d4e'}
                      strokeWidth={2}
                      dot={{ fill: LEVEL_COLORS[level] || '#538d4e', r: 3 }}
                      connectNulls
                    />
                  ))}
                </LineChart>
          ...
          {Object.keys(avgByStimulus).length > 0 && (
            <div className="card">
              <h3 className="text-sm text-text-muted font-medium uppercase tracking-wider mb-3">
                Average by Type
              </h3>
              <div className="space-y-2">
                {Object.entries(avgByStimulus).map(([type, avg]) => (
                  <div key={type} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-24 h-2 bg-secondary-surface rounded-full overflow-hidden">
                        <div
                          className="h-full rounded-full"
                          style={{
                            width: `${avg}%`,
                            backgroundColor: avg >= 80 ? '#538d4e' : avg >= 60 ? '#c4a035' : '#b85c4e',
                          }}
                        />
                      </div>
                      <span className="text-sm font-medium text-primary-500 w-10 text-right">{avg}%</span>
                    </div>
                  </div>
                ))}
              </div>
                ...
                <div key={session.id} className="card flex items-center justify-between">
                  <div>
                    <div className="font-bold text-text-primary">
                      {session.nLevel}-Back
                      {session.adaptive && (
                        <span className="text-xs bg-[#8b6eae]/15 text-[#8b6eae] px-1.5 py-0.5 rounded ml-2">
                          Adaptive
                        </span>
                      )}
                    ...
                    <div className="text-xs text-text-muted mt-1">
                      ...
                      {session.maxCombo > 0 && ` · ${session.maxCombo}x combo`}
                  ...
                  <div className="flex items-center gap-3">
                    <div className="text-right">
                      ...
                      <div className="text-xs text-[#c4a035]">+{session.xpEarned} XP</div>
                    </div>
                    <button
                      onClick={() => setDeleteTarget(session.id)}
                      className="text-text-muted/50 hover:text-[#b85c4e] transition-colors p-1"
                    >
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 px-4">
          <div className="card max-w-sm w-full space-y-4">
            <h3 className="text-lg font-bold text-text-primary">Delete this session?</h3>
            <p className="text-sm text-text-muted">This action cannot be undone. XP earned from this session will not be recalculated.</p>
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 px-6 py-3 bg-[#b85c4e] hover:bg-[#c46b5e] text-white font-semibold rounded-xl transition-all duration-200 active:scale-[0.98] disabled:opacity-50"
                disabled={deleting}
              >
